...
describe("BattleMove integration", () => {
    ...
    describe("BattleMove with diceBonus", () => {
        it("should roll dice + diceBonus dice when diceBonus=1", () => {
            // Setup: 2 base dice + 1 bonus = 3 total dice rolled
            // Return 3 infantry hits to verify 3 dice were rolled
            const { gameState, attacker, target } = setupBattle(
                [RESULT_INFANTRY, RESULT_INFANTRY, RESULT_INFANTRY],
                4,
                4
            );

            // Create BattleMove with 2 base dice and 1 diceBonus
            const move = new BattleMove(attacker, target, 2, false, 1);
            move.execute(gameState);

            // Assert: 3 hits dealt (proving 3 dice were rolled)
            expect(gameState.getUnitCurrentStrength(target)).toBe(1);
        });

        it("should roll dice + diceBonus dice when diceBonus=2", () => {
            // Setup: 2 base dice + 2 bonus = 4 total dice rolled
            const { gameState, attacker, target } = setupBattle(
                [RESULT_INFANTRY, RESULT_INFANTRY, RESULT_INFANTRY, RESULT_INFANTRY],
                4,
                4
            ...
            // Create BattleMove with 2 base dice and 2 diceBonus
            const move = new BattleMove(attacker, target, 2, false, 2);
            move.execute(gameState);

            // Assert: 4 hits dealt (proving 4 dice were rolled)
            expect(gameState.getUnitCurrentStrength(target)).toBe(0);
        });

        it("should roll just dice when diceBonus=0 (default)", () => {
            // Setup: 2 base dice + 0 bonus = 2 total dice rolled
            const { gameState, attacker, target } = setupBattle(
                [RESULT_INFANTRY, RESULT_INFANTRY],
                4,
                4
            ...
            // Create BattleMove with 2 base dice and 0 diceBonus (default)
            const move = new BattleMove(attacker, target, 2);
            move.execute(gameState);

            // Assert: 2 hits dealt (proving 2 dice were rolled)
            expect(gameState.getUnitCurrentStrength(target)).toBe(2);
        });

        it("should handle diceBonus with flag results", () => {
            // Setup: 1 base die + 1 bonus = 2 dice, both return flags
            const { gameState, attacker, target } = setupBattle(
                [RESULT_FLAG, RESULT_FLAG],
                4,
                4
            ...
            // Create BattleMove with 1 base die and 1 diceBonus
            const move = new BattleMove(attacker, target, 1, false, 1);
            move.execute(gameState);

            // Assert: RetreatPhase pushed with 2 flags (proving 2 dice were rolled)
            expect(gameState.activePhase).toBeInstanceOf(RetreatPhase);
            const retreatPhase = gameState.activePhase as RetreatPhase;

            // With 2 flags, unit must retreat 2 hexes
            // Target at (5,4) -> distance 2 north gives: (5,2), (6,2), (7,2)
            expect(retreatPhase.availableRetreatHexes.length).toBe(3);
            expect(retreatPhase.availableRetreatHexes).toContainEqual(new HexCoord(5, 2));
            expect(retreatPhase.availableRetreatHexes).toContainEqual(new HexCoord(6, 2));
